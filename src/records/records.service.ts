...
export class RecordsService implements IRecordsService {
  ...
  private toDto(x: any): RecordResponseDto { return x as RecordResponseDto; }
  ...
  private async assertDoctorId(doctorId: number) {
    ...
    if (!d || d.role !== 'doctor') throw new BadRequestException('doctorId must reference a doctor user');
  }

  // POST /medical-records/:medicalRecordId/records
  async createForMedicalRecord(medicalRecordId: number, actor: UserCtx, dto: CreateRecordDto) {
  ...
  async listForMedicalRecord(medicalRecordId: number, q: QueryRecordDto, currentUser: UserCtx) {
    const patientId = await this.getMRPatientId(medicalRecordId);
